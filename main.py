...
# ==================== VIP 配置读取及逻辑 开始 ====================
        ...
        if hasattr(self, "config") and self.config:
            ...
            try:
                vip_min_luck = int(fortune_config.get("VIP_MIN_LUCK_VALUE", 80))
            except ValueError:
                vip_min_luck = 80
...
# ==================== VIP 配置读取及逻辑 结束 ====================
    ...
    async def jrysd(self, event: AstrMessageEvent, target_id: str):
        ...
        yield event.plain_result(f"⚠️ 未找到用户 {target_id_str} 今日的运势记录，可能尚未抽取。") 
    @filter.command("jryscfg")
    async def jryscfg(self, event: AstrMessageEvent):
        """[调试] 查看当前内存中实际加载的配置状态"""
        if hasattr(self, "config"):
            import json
            try:
                config_str = json.dumps(self.config, ensure_ascii=False, indent=2)
                yield event.plain_result(f"当前插件配置内存状态:\n{config_str}")
            except Exception as e:
                yield event.plain_result(f"配置读取异常: {self.config}\n错误: {e}")
        else:
            yield event.plain_result("❌ 未能检测到 self.config 属性，框架可能未正确注入配置！")
